...
from .common import EnvironmentRegistry, api_auth, job_run_credentials, submit_host_job
...
class PackageRepoService:
    ...
    def _do_push(self, ctx: JobContext) -> None:
        ...
        with self._mgmt_client_factory(host, read_only=False, domain=domain, **auth) as client:
            ...
            repo_path = (
                self._staging_dir if self._staging_dir.endswith("/") else self._staging_dir + "/"
            )
