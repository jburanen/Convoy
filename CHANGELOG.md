...
A tenant is **not** a new kind of object: it is stored as the environment's one management server (`env_hosts`, migration v29 adding `mgmt_api_context` for the tenant UUID) with the key held in an ordinary credential set assigned to it. Discovery, package-repository pushes and every other Management API caller are unchanged, because a `Host` plus an API key is exactly what they already need — the only code that learned about Smart-1 Cloud is the URL builder, which inserts the tenant path segment, and the new service. `Host` validates that segment to a bare identifier: it is interpolated into the URL the environment's API key is sent to, so a value carrying a slash or a scheme could redirect that key. TLS verification now defaults **on** when a tenant path is present — a public Check Point host with a real certificate, unlike the self-signed on-prem server the old blanket default was written for. Connecting proves the credentials by logging in *before* storing anything, so a mistyped UUID or a revoked key leaves no half-configured server row for the next job to trip over, and it runs synchronously with an audit record, like credential CRUD. Firewalls in these environments are untouched: still reached over SSH, still patched the same way. Closes two Punch List items — the S1C API endpoint URL, and a separate bootstrapping flow for API-only scenarios

**First-run configuration reaches a redeployed instance again.** Since v0.80.0 first run has opened the full Manage Environments modal — every option for the environment present while it is being named, not just a rename. Whether it opened at all was gated on a flag in `localStorage`, which lives in the browser while a clean install returns the *deployment* to exactly the first-run state: once dismissed, no later `deploy.sh --reset` on that machine ever showed the guidance again, which is the one case it exists for. The gate is now the deployment's own state — one environment, still named `default`, no servers, packages or credential sets, and every setting still at its default. Configuring anything (a rename, a type, credential storage, skip-verify) makes it false on the next load, so it retires itself without remembering anything, and all four controls now also hide the note the moment they are used

## v0.81.0

Two Spark fixes, related more closely than they look.

**A transfer leaves `bashUser` the way it found it.** `spark.scp` turned it on before the upload and off afterwards unconditionally, so a device the operator had deliberately left with shell access came out of an unrelated upload with it silently revoked. It now detects the existing state and skips both halves when it was already on (operator-specified 2026-08-27). No probe command was needed: `bashUser on` is what makes the account's login shell bash, so such a device drops an SSH login straight into expert with no clish prompt — exactly what `enter_expert()` already reports by returning False. The session's own landing prompt is the evidence. Skipping the redundant enable just saves a command; skipping the disable is the half that matters.

**`fw ver` runs in a shell that can find it.** The post-install verification failed `bash: line 1: fw: command not found` (rc 127) on a firewall whose upgrade had genuinely succeeded — the device was on the new build, only the step confirming it could not read the version back, so the job was reported as failed. The shell is not the variable: `fw ver` is valid in clish and expert alike (operator-confirmed, by hand, in both). What matters is Check Point's PATH, which the profile an interactive *login* shell sources sets up — and `exec_command` gets neither interactive nor login. The operator running it at that same moment worked for precisely that reason. It now goes through `bash -lc` when the login shell is bash, stays bare when it is clish, and falls back to `clish -c` once if that fails, since one call decides whether a completed upgrade is called a success. Both call sites use it — the post-install check and the Firewalls-tab Refresh, which would have failed identically. And the first fix above makes this far more reachable: a device that keeps `bashUser` on now hands every later job a bash login shell

## v0.80.0

The Manage Environments modal's two checkboxes (Multi-Domain, API-only) become one **type** picker inline with the environment name: SmartCenter (SMS), Multi-Domain (MDS) or Smart-1 Cloud, with a note under the row explaining whichever is selected. The two flags were never really independent - the three combinations an operator actually deploys are exactly these, while a checkbox pair also offered "MDS + API-only", which nothing here is built to drive. Applied through a new `POST /api/environments/{env}/type` that sets both flags in one call: driving a single picker through the two existing endpoints in sequence could leave an environment in precisely the combination the picker exists to prevent, permanently if the second call failed. `api_only` wins on read, so a legacy row carrying both flags shows as Smart-1 Cloud and is normalized the next time the picker is touched. First run opens this same modal too, with an added note carrying the guidance the old rename-only welcome dialog held (one environment per management system; one for the whole MDS, not per domain) - so every option for the environment is in front of the operator while they are naming it, instead of the name being settled first and type and credential storage discovered later. That dialog is gone. Its subtlest rule is kept: only an explicit choice (renaming, or setting the type) is remembered per browser, so a stray Escape or backdrop click cannot suppress first-run guidance forever
